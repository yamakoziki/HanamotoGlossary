...
The app supports up to **4 languages** in the viewer:
...
- All logic is vanilla JS inside `admin/admin.html` (~1820 lines). There is no separate JS/CSS file.
- **Do not edit `docs/index.html` directly** — it is overwritten each time Step 5 runs. All viewer UI/CSS changes must be made inside `buildIndexHtml()` (line ~1070) in admin.html, then regenerated.
- Translation uses `claude-sonnet-4-20250514` via `callAnthropicApi()` (line ~609). Calls are sequential with a 1-second delay between requests for rate limiting. Translation targets English and the active 3rd language (`lang3Code`).
- The generated `docs/index.html` embeds the viewer UI inline and all glossary data as JS variables (`const TERMS = [...]` and `const CATEGORIES = [...]`) — produced by `buildIndexHtml()` (line ~1070). Images are embedded as base64 `dataUrl` strings. Maximum 5 images per term.
...
- The export round-trip (`exportToExcel()`, line ~1711) recovers data from a generated `index.html` by parsing the `const TERMS =` and `const CATEGORIES =` variable declarations via `extractJsonFromHtml()` (line ~1690) — so only HTML generated by this tool can be round-tripped.
